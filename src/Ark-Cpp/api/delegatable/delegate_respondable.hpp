...
#ifndef DELEGATE_RESPONDABLE_HPP
#define DELEGATE_RESPONDABLE_HPP
...
*   @brief:	Model for Delegate Search API Response
**************************************************/
struct Search : public search_t, Printable
{
public:
	Search(
...
*		operators: Voter& operator[](size_t index), Voter& operator[](size_t index)
*
*   @brief:	Model for Delegate Voters API Response
**************************************************/
struct Voters : public Printable
{
public:
	size_t count;
	ARK::Voter *const voters;

	explicit Voters(size_t c) : count(c), voters(new ARK::Voter[c]) {}
...
*   @brief: Model for Delegate Forging Totals API Response
**************************************************/
struct ForgedByAccount : public forged_by_account_t, Printable
{
public:
	ForgedByAccount(
...
*   @brief:	Model for Next 10 Forging Delegate Publickeys API Response
**************************************************/
struct NextForgers : public next_forgers_t, Printable
{
public:
	NextForgers(
			int newCB,
	...
	{
		Serial.println(newCB);
		this->currentBlock= newCB;
		this->currentSlot = newCS;
		for (auto i = 0; i < 10; ++i)
		{
