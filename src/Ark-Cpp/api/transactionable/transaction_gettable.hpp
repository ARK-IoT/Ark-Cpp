...
#ifndef TRANSACTION_GETTABLE_HPP
#define TRANSACTION_GETTABLE_HPP
...
protected:
	...
	**************************************************/
			...
			const Hash &txID)
	{
		...
		auto callback = netConnector.cb(uri);
		return vendorFieldfromJSON(callback);
	}
	/*************************************************
	*	@brief: vendorFieldfromJSON(const char *const jsonStr)
	**************************************************/
	const char*  vendorFieldfromJSON(const char *const jsonStr)
	{
		ARK::Utilities::JSONParser parser(jsonStr, strlen(jsonStr));
