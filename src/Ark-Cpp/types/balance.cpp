...
Balance::Balance() : arktoshi_(), ark_()
{
  this->setBalance("0");
};


Balance::Balance(const char* const balanceStr) : arktoshi_(), ark_()
{
  bool isNumeric = true;
...
Balance::Balance(const Balance& other) : arktoshi_(), ark_()
{
...
Balance::Balance(Balance&& other) : arktoshi_(), ark_()
{
  strcpy(arktoshi_, other.arktoshi_);
  strcpy(ark_, other.ark_);
  other.arktoshi_[0] = '\0';
...
Balance& Balance::operator=(Balance&& other)
{
  if (this != &other)
  {
    strcpy(arktoshi_, other.arktoshi_);
...
const char* Balance::ark() const { return ark_; };
const char* Balance::arktoshi() const { return arktoshi_; };


void Balance::setArktoshi(const char* const balanceStr)
{
    strncpy(arktoshi_, balanceStr, ARKTOSHI_SIZE);
};


void Balance::setArk(const char* const balanceStr)
{
...
void Balance::setBalance(const char* const _balanceStr)
{
