#include "gtest/gtest.h"

#include "api.h"

TEST(peer, test_peer) {
	ARK::API::Manager _arkManager(ARK::Constants::Networks::Devnet::model);

	const auto peer = _arkManager.peer("167.114.29.55", 4002);
	ASSERT_STREQ("", peer.ip());
	ASSERT_EQ(0, peer.port());
	ASSERT_STREQ("", peer.version());
	ASSERT_EQ(0, peer.errors());
	ASSERT_STREQ("", peer.os());
	ASSERT_STREQ("", peer.height());
	ASSERT_STREQ("", peer.status());
	ASSERT_EQ(0, peer.delay());


	/*  ==================================  */
	/*  ==================================  */
	/*    BROKEN: fix for large callbacks  */
	/*    Peers callback is ~10,000 bytes  */
	//  String peersDescription = _arkManager.peers().description();
	//    Serial.println("peersDescription: ");
	//    Serial.println(peersDescription);
	//    Serial.println("\n=====\n");
	//    delay(50);
	/*  ==================================  */
	/*  ==================================  */

	/*  ==================================  */
	const auto version = _arkManager.peerVersion();
	ASSERT_STREQ("", version.version());
	ASSERT_STREQ("", version.build());

}
