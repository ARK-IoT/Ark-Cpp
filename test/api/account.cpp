...
TEST(api, test_account_balance)
{
	...
	auto balanceCon = account_balance.confirmed().getValue();
	ASSERT_NE(0.0, balanceCon);
...
TEST(api, test_account_delegates_fee)
{
	...
	ASSERT_EQ(25.0, delegates_fee.getValue());
}
...
TEST(api, test_account)
{
	...
	ASSERT_NE(0.0, unconfirmed_balance.getValue());
	const auto& balance = account.balance();
	ASSERT_NE(0.0, balance.getValue());
